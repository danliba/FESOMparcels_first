...
## Let's interpolate the real deal
...
for itime in range(0,dat1.shape[0]):
    for level in range(0,dat1.shape[1]):
        ...
        u_grid[:,:,level,itime] = data_grid_flat.reshape((292, 72))
    #print(itime)
    print(dat1.time.values[itime].astype('datetime64[D]'))
    #u_grid[:,:,level] = data_grid_flat.reshape((292, 72))

    
#date_only = dat1.time.values.astype('datetime64[D]')
#print(date_only)
#%% plot
...
im = ax.pcolormesh(lon_grid, lat_grid, u_grid[:,:,-1,0], shading='auto', cmap=cmap) 
